...
export default function TagInput({ value = [], onChange, maxTags = 5, suggestions = [] }) {
  ...
  const filteredSuggestions = useMemo(() => {
    ...
    return term
      ? list.filter(t => t.toLowerCase().includes(term) && !value.includes(t)).slice(0, 6)
  ...
  return (
    <div>
      <div className="d-flex flex-wrap gap-2 mb-2">
        {value.map((t) => (
          <span key={t} className="badge text-bg-secondary d-flex align-items-center gap-2">
            {t}
            <button
              type="button"
